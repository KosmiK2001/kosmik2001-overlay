...
RDEPEND="
	${COMMON_DEPEND}
	>=app-eselect/eselect-opengl-1.4.1
...
src_prepare() {
	...
	patchelf --add-needed libpthread.so.0 libGL.so.340.108 || die
...
src_install() {
	nvidia-drivers_libs_install() {
		...
		# eselect-opengl provider (switched via /etc/env.d/000opengl LDPATH)
		if use X; then
			dodir /usr/$(get_libdir)/opengl/nvidia/lib
			dosym /usr/$(get_libdir)/nvidia/libGL.so.1 \
				/usr/$(get_libdir)/opengl/nvidia/lib/libGL.so.1
			dosym /usr/$(get_libdir)/nvidia/libGL.so \
				/usr/$(get_libdir)/opengl/nvidia/lib/libGL.so
			# glvnd shims: eselect-opengl points the system
			# libGLX.so.0.0.0 / libOpenGL.so.0.0.0 at these when the
			# nvidia provider is selected, so glvnd-linked applications
			# (mesa-utils etc) get the legacy libGL's GLX/GL entrypoints
			dosym /usr/$(get_libdir)/nvidia/libGL.so.1 \
				/usr/$(get_libdir)/opengl/nvidia/lib/libGLX.so.0
			dosym /usr/$(get_libdir)/nvidia/libGL.so.1 \
				/usr/$(get_libdir)/opengl/nvidia/lib/libOpenGL.so.0
		fi
	}
	...
	if use X; then
		...
		newenvd - "010nvidia" <<-EOF
			LDPATH="$( IFS=:; echo "${nvidia_ldpaths[*]}" )"
		EOF
	fi

	# load the module at boot, before opengl-detect picks the provider
	printf "%s\n" nvidia > "${T}/nvidia-modules-load.conf" || die
	insinto /usr/lib/modules-load.d
	doins "${T}/nvidia-modules-load.conf"
